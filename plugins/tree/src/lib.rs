...
fn normalize_tree_node(
    ...
    *next_id += 1;
    nodes.push(InternalNode {
        ...
        label_text: normalize_tree_label(node.name.clone()),
        length: node.length,
        resolved_length: 0.0,
    });
    ...
    for child in &node.children {
        let (child_id, child_has_explicit_non_root_length) =
            normalize_tree_node(child, nodes, next_id, Some(id), false);
...
fn normalize_raw_tree(
    ...
    let mut next_id = 0;
    let (root_id, has_explicit_non_root_length) =
        normalize_tree_node(&raw_tree, &mut nodes, &mut next_id, None, true);
