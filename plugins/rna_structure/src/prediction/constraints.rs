use crate::prediction::{FoldError, MIN_PAIR_SPAN, sequence::Base};
...
impl Constraints {
    ...
    /// Returns the first forced pair that encloses fewer than three bases.
    pub(crate) fn first_sharp_pair(&self) -> Option<(usize, usize)> {
        self.positions
            .iter()
            .enumerate()
            .find_map(|(left, constraint)| match *constraint {
                Constraint::Paired(right) if left < right && right - left < MIN_PAIR_SPAN => {
                    Some((left, right))
                }
                _ => None,
            })
...
mod tests {
    use super::*;
    use crate::prediction::sequence::normalize;
    ...
    #[test]
    fn forced_position_queries_respect_range_boundaries() {
        let bases = normalize(b"AUGCAUGC").unwrap();
        let constraints = Constraints::parse(b"??(..)??", &bases).unwrap();

        assert_eq!(constraints.pair_boundary_after(0), 2);
        assert_eq!(constraints.pair_boundary_after(2), 5);
        assert_eq!(constraints.first_forced_position_at_or_after(0), 2);
        assert_eq!(constraints.first_forced_position_at_or_after(2), 2);
        assert_eq!(constraints.first_forced_position_at_or_after(3), 5);
        assert_eq!(constraints.first_forced_position_at_or_after(6), 8);

        assert!(constraints.range_can_unpair(0, 2));
        assert!(!constraints.range_can_unpair(0, 3));
        assert!(constraints.range_can_unpair(2, 2));
        assert!(!constraints.range_can_unpair(2, 3));
        assert!(constraints.range_can_unpair(6, 8));
        assert!(constraints.can_pair(2, 5));
        assert!(!constraints.can_pair(2, 4));
    }
}
