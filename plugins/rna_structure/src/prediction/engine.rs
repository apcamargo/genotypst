...
use crate::prediction::{
    FoldError, FoldOptions, MIN_PAIR_SPAN,
...
mod tests {
    ...
    #[test]
    fn traceback_reports_a_missing_complete_state() {
        let charts = Charts::<i32>::new(1);

        assert_eq!(
            traceback(&charts, 1),
            Err(FoldError::InternalInvariant("missing complete trace"))
        );
    }
}
...
pub(crate) fn fill_charts<M: ScoringModel>(
    ...
    for j in 0..length {
        ...
        if !options.allow_sharp_turns {
            while first_right.is_some_and(|right| right - j < MIN_PAIR_SPAN) {
        ...
        if use_cube_pruning && j + 1 < length {
            ...
            while filled < options.beam_size || current == previous {
...
pub(crate) fn run_circular_vienna(
    filled: Filled<crate::prediction::model::ViennaModel>,
    constraints: Option<&Constraints>,
    length: usize,
    ...
    let mut best_score = if constraints.is_none_or(|value| value.range_can_unpair(0, length)) {
        crate::prediction::model::ViennaModel::circular_unpaired_score(length)
    } else {
        i32::MIN
    };
    ...
    for first in &inventory.pairs {
        ...
        for a in 0..=remaining {
            for b in 0..=remaining - a {
                ...
                let loop_score = crate::prediction::model::ViennaModel::circular_internal_score(
                    first.left,
                    first.right,
                    p,
                    q,
    ...
    if prefix[multi_index] != i32::MIN {
        let score = prefix[multi_index]
            + crate::prediction::model::ViennaModel::circular_multiloop_closing_score();
        if score > best_score {
            best_score = score;
            best_roots.clear();
