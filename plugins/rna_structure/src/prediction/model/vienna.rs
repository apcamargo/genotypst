use crate::prediction::{DangleModel, sequence::Base};
...
mod tests {
    ...
    #[test]
    fn special_hairpins_use_their_reference_scores() {
        assert_eq!(ViennaModel::hairpin_core(3, 1, 0, 0, Some(b"CAACG")), 680);
        assert_eq!(ViennaModel::hairpin_core(4, 1, 0, 0, Some(b"CAACGG")), 550);
        assert_eq!(
            ViennaModel::hairpin_core(6, 5, 0, 0, Some(b"ACAGUACU")),
            280
        );
    }

    #[test]
    fn ordinary_hairpins_and_degree_two_loops_use_their_reference_scores() {
        assert_eq!(ViennaModel::hairpin_core(4, 1, 1, 1, None), 410);
        assert_eq!(ViennaModel::degree_two_energy(0, 0, 1, 1, 0, 0, 0, 0), -240);
        assert_eq!(ViennaModel::degree_two_energy(1, 0, 1, 1, 0, 0, 0, 0), 140);
    }

    #[test]
    fn dangle_modes_change_multiloop_and_external_scores() {
        let both = ViennaModel {
            dangles: DangleModel::Both,
            sequence: Vec::new(),
        };
        let none = ViennaModel {
            dangles: DangleModel::None,
            sequence: Vec::new(),
        };
        assert_eq!(both.ml_stem(1, Some(0), Some(0)), -140);
        assert_eq!(none.ml_stem(1, Some(0), Some(0)), -90);
        assert_eq!(both.external_stem(1, Some(0), Some(0)), -50);
        assert_eq!(none.external_stem(1, Some(0), Some(0)), 0);
    }
}
