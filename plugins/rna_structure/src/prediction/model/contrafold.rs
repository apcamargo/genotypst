...
impl ScoringModel for ContraFoldModel {
    ...
    fn m1(&self, i: usize, j: usize, nucs: &[u8]) -> f64 {
        Self::junction_a(j, i, nucs) + Self::pair(nucs[i], nucs[j]) + MULTI_PAIRED
    }

    fn external_paired(&self, i: usize, j: usize, nucs: &[u8]) -> f64 {
        Self::junction_a(j, i, nucs) + EXTERNAL_PAIRED + Self::pair(nucs[i], nucs[j])
    }
...
mod tests {
    ...
    #[test]
    fn primitive_scores_match_independent_reference_values() {
        let model = ContraFoldModel;
        assert!((ContraFoldModel::pair(0, 3) - 0.597_911_99).abs() < 1e-12);
        assert!((model.multi_unpaired(3) + 0.59499).abs() < 1e-12);
        assert!((model.external_unpaired(4) + 0.038_916).abs() < 1e-12);
    }

    #[test]
    fn single_loop_cache_uses_each_boundary_policy() {
        assert!((SINGLE_CACHE[0][1] + 2.399_548_472).abs() < 1e-12);
        assert!((SINGLE_CACHE[1][1] + 1.151_228_810_5).abs() < 1e-12);
        assert!((SINGLE_CACHE[30][30] + 6.978_013_930_31).abs() < 1e-12);
        assert!((SINGLE_CACHE[1][29] + 9.813_701_408_01).abs() < 1e-12);
    }
}
