mod contrafold;
mod params {
    pub(in crate::prediction::model) mod contrafold;
    pub(in crate::prediction::model) mod vienna;
}
mod vienna;
...
/// Integrity guards for the two energy parameter sets.
///
/// These checksums cover parameter entries that the folding examples may not
/// use. A damaged entry could therefore leave those examples passing while
/// changing the model. After an intentional parameter update, copy the failing
/// test's reported `left` hash into the expected value.
#[cfg(test)]
mod parameter_tests {
    ...
    fn contrafold_parameter_checksum_matches_reference() {
        ...
        assert_eq!(
            hash, 0xc4f6_86bd_8764_aaf5,
            "ContraFold parameters changed; see the module comment before updating this hash"
        );
    }
    ...
    fn vienna_parameter_checksum_matches_reference() {
        ...
        assert_eq!(
            hash, 0x2d15_0dd4_97ad_583a,
            "Vienna parameters changed; see the module comment before updating this hash"
        );
    }
}
