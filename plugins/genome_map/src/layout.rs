//! Genome map layout: label packing, vertical geometry, and leader routing.
...
/// Variant ordering is significant: the sweep-line algorithm sorts events by
/// `(x_pt, kind, label_index)`, and correctness requires `Start < Query < End`
/// so that at equal x-positions, insertions are processed before queries,
/// and queries before removals.
...
/// is required.
fn fits_in_level(intervals: &[(f64, f64)], left: f64, right: f64, spacing: f64) -> bool {
...
fn assign_levels(labels: &mut [WorkingLabel], spacing: f64) -> usize {
    ...
    for label in labels.iter_mut() {
        ...
        for (idx, level) in levels.iter_mut().enumerate() {
            if fits_in_level(&level.intervals, left, right, spacing) {
                label.level = idx;
...
pub(crate) fn compute_layout(request: &LayoutRequest) -> Result<LayoutResponse, String> {
...
#[cfg(test)]
fn vertical_geometry(layout: &LayoutGeometry, level: usize) -> (f64, f64) {
    let top_pt = layout.level_base_top_pt - level as f64 * layout.level_step_pt;
    let bottom_pt = top_pt + layout.label_height_pt;
    (top_pt, bottom_pt + layout.underline_offset_pt)
}
...
mod tests {
    ...
    fn assert_layout_matches_reference_levels(request: &LayoutRequest) {
        ...
        for (actual, expected) in response.labels.iter().zip(&working) {
            let (top_pt, underline_y_pt) = vertical_geometry(&layout, expected.level);
            assert_eq!(actual.source_index, expected.source_index);
            assert_close(actual.top_pt, top_pt);
            assert_close(actual.underline_y_pt, underline_y_pt);
        }
    }
    ...
    fn layout_matches_reference_equal_spans() {
        ...
        let request = LayoutRequest {
            ...
            label_line_distance_pt: LABEL_LINE_DISTANCE_PT,
            label_track_gap_pt: LABEL_TRACK_GAP_PT,
            label_leader_offset_pt: LABEL_LEADER_OFFSET_PT,
            labels,
        };
    ...
    fn layout_matches_reference_just_touching() {
        let spacing = 1.0;
        let labels = vec![
            simple_measured_label(15.0, 10.0, 15.0),
            simple_measured_label(36.0, 10.0, 36.0),
        ];
        let request = LayoutRequest {
            label_height_pt: LABEL_HEIGHT_PT,
            label_horizontal_gap_pt: spacing,
            label_vertical_gap_pt: 4.0,
            label_line_distance_pt: LABEL_LINE_DISTANCE_PT,
            label_track_gap_pt: LABEL_TRACK_GAP_PT,
            label_leader_offset_pt: LABEL_LEADER_OFFSET_PT,
    ...
    fn layout_differential_randomized() {
        ...
        for trial in 0..200 {
            ...
            let request = LayoutRequest {
                label_height_pt: label_height,
                label_horizontal_gap_pt: h_gap,
                label_vertical_gap_pt: v_gap,
                label_line_distance_pt: line_dist,
                label_track_gap_pt: track_gap,
                label_leader_offset_pt: leader_offset,
                labels,
            };
    ...
    #[test]
    fn layout_labels_json_round_trip_moved() {
        let json = r#"{
        ...
        let response_bytes = crate::layout_labels(json.as_bytes()).unwrap();
        let response: LayoutResponse = serde_json::from_slice(&response_bytes).unwrap();
